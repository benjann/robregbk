...
The case $\alpha=0.5$ corresponds to \stsc{LAD} estimate. Assume the model
\[
...
The most usual way to measure the \textquotedblleft
...
Because of this vulnerability to bad leverage points, generalized M-estimators
...
that \stsc{LS} estimation is based on the minimization of the variance of the
...
Relying on this idea, Rousseeuw (1983) introduced the \emph{Least Trimmed
...
one, the LTS-estimator tends to the \stsc{LS} estimator. In contrast, if $\alpha
...
us consider once again the \stsc{LS} estimation. For \stsc{LS} estimation, we actually are
...
the same philosophy as the one underlying the \stsc{LS} estimation, but introduces
...
\subsubsection{Comparison of $\sthat{\underline{\boldsymbol\beta}}_{\mathrm{LS}}$ and $\sthat{\underline{\boldsymbol\beta}}_{\mathrm{S};\rho_{0}}$}
...
\subsubsection{Comparison of $\sthat{\boldsymbol\beta}_{\mathrm{S};\rho_{0}}$ and $\sthat{\boldsymbol\beta}_{\mathrm{MM};\rho_{0},\rho}$}
